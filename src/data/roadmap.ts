import type { RoadmapItem } from "@/types/roadmap"

export const roadmapItems: RoadmapItem[] = [
  {
    id: "roadmap-sentra-onboarding",
    ventureId: "venture-sentra",
    title: "Activation Analytics Recovery",
    description:
      "Restore activation measurement so Sentra can keep scaling from a higher-confidence base.",
    timeframe: "now",
    goal: "Restore trustworthy activation signal before the next growth experiment.",
    status: "active",
    ownerId: "user-sarah-chen",
    linkedIssueIds: [
      "issue-sentra-analytics-ingestion",
      "issue-sentra-event-taxonomy",
    ],
    progress: 66,
    confidence: 86,
    manualProgress: 66,
    manualConfidence: 78,
    confidenceTrend: "improving",
    impact: "high",
    riskLevel: "medium",
    targetMetric: "Reliable activation events above 95%",
    validationGateId: "gate-sentra-activation",
    betType: "validation",
    expectedEvidence: ["Taxonomy mapped", "Ingestion rate > 95%"],
    aiInsightIds: ["insight-sentra-analytics-risk"],
    createdAt: "2026-05-01T09:00:00.000Z",
    updatedAt: "2026-05-24T10:00:00.000Z",
  },
  {
    id: "roadmap-sentra-referrals",
    ventureId: "venture-sentra",
    title: "Mobile Referral Expansion",
    description:
      "Extend referral loops into mobile onboarding and workspace invites.",
    timeframe: "next",
    goal: "Increase qualified invited workspace creation.",
    status: "active",
    ownerId: "user-lina-haddad",
    linkedIssueIds: [
      "issue-sentra-mobile-conversion-copy",
      "issue-sentra-referral-abuse-guardrails",
    ],
    progress: 74,
    confidence: 88,
    confidenceTrend: "improving",
    impact: "high",
    riskLevel: "low",
    targetMetric: "Referral-sourced activation +9%",
    validationGateId: "gate-sentra-activation",
    betType: "growth",
    expectedEvidence: ["Referral-sourced activation lift verified"],
    aiInsightIds: [],
    createdAt: "2026-05-02T09:00:00.000Z",
    updatedAt: "2026-05-24T10:00:00.000Z",
  },
  {
    id: "roadmap-reson8-retention",
    ventureId: "venture-reson8",
    title: "Creator Retention Signal Validation",
    description:
      "Decide whether repeat creator collaboration is strong enough to justify continued buildout.",
    timeframe: "now",
    goal: "Narrow Reson8 around retained-creator proof before expanding onboarding scope.",
    status: "at-risk",
    ownerId: "user-sarah-chen",
    linkedIssueIds: [
      "issue-reson8-retention-assumptions",
      "issue-reson8-positioning-interviews",
      "issue-reson8-message-threshold",
      "issue-reson8-onboarding-variant",
    ],
    progress: 41,
    confidence: 35,
    manualProgress: 41,
    manualConfidence: 23,
    confidenceTrend: "declining",
    impact: "high",
    riskLevel: "high",
    targetMetric: "Weekly retained-creator cohort above 40%",
    validationGateId: "gate-reson8-retention",
    betType: "validation",
    evidenceSignalIds: [
      "signal-reson8-weekly-cohort-weak",
      "signal-reson8-interviews-fail",
      "signal-reson8-buildout-capacity-cost",
    ],
    expectedEvidence: [
      "Weekly creator cohort clears 40%",
      "Creator interview urgency improves",
      "Onboarding scope remains narrow until proof improves",
    ],
    decisionImpact: "narrow",
    aiInsightIds: ["insight-reson8-validation-criteria"],
    createdAt: "2026-05-05T09:00:00.000Z",
    updatedAt: "2026-05-24T10:00:00.000Z",
  },
  {
    id: "roadmap-internal-meeting-intelligence",
    ventureId: "venture-internal-ops",
    title: "Meeting Intelligence Rollout",
    description:
      "Turn venture review meetings into structured summaries and follow-up issue capture.",
    timeframe: "now",
    goal: "Reduce missed follow-ups after weekly venture operating meetings.",
    status: "active",
    ownerId: "user-maya-rodriguez",
    linkedIssueIds: [
      "issue-internal-meeting-summary",
      "issue-internal-meeting-action-sync",
      "issue-internal-meeting-rollout-notes",
    ],
    progress: 64,
    confidence: 71,
    manualConfidence: 71,
    confidenceTrend: "stable",
    impact: "medium",
    riskLevel: "low",
    targetMetric: "Weekly follow-up misses below 5%",
    validationGateId: "gate-internal-ops-leverage",
    betType: "leverage",
    expectedEvidence: ["Review summary prep time cut 20%"],
    aiInsightIds: [],
    createdAt: "2026-05-08T09:00:00.000Z",
    updatedAt: "2026-05-24T10:00:00.000Z",
  },
  {
    id: "roadmap-reson8-pricing-signal",
    ventureId: "venture-reson8",
    title: "Creator Pricing Signal Discovery",
    description:
      "Hold monetization discovery until retention confidence is strong enough to justify pricing work.",
    timeframe: "later",
    goal: "Identify whether pricing clarity should follow the retention decision.",
    status: "planned",
    ownerId: "user-omar-khaled",
    linkedIssueIds: [],
    progress: 12,
    confidence: 57,
    confidenceTrend: "stable",
    impact: "medium",
    riskLevel: "medium",
    targetMetric: "Pricing discovery gate defined",
    aiInsightIds: [],
    createdAt: "2026-05-18T09:00:00.000Z",
    updatedAt: "2026-05-24T10:00:00.000Z",
  },
  {
    id: "roadmap-internal-hiring-workflow",
    ventureId: "venture-internal-ops",
    title: "Founder Hiring Workflow Automation",
    description:
      "Map founder candidate handoffs without turning studio hiring into a heavy workflow system.",
    timeframe: "later",
    goal: "Reduce manual coordination in early founder hiring loops.",
    status: "planned",
    ownerId: "user-maya-rodriguez",
    linkedIssueIds: ["issue-internal-hiring-automation"],
    progress: 22,
    confidence: 68,
    confidenceTrend: "improving",
    impact: "medium",
    riskLevel: "low",
    targetMetric: "Hiring handoff misses below 10%",
    aiInsightIds: [],
    createdAt: "2026-05-20T09:00:00.000Z",
    updatedAt: "2026-05-24T10:00:00.000Z",
  },
  {
    id: "roadmap-sentra-referral-foundation",
    ventureId: "venture-sentra",
    title: "Referral Foundation Release",
    description:
      "Ship the initial referral activation loop and confirm cohort quality before incentive expansion.",
    timeframe: "now",
    goal: "Prove referral-sourced workspaces activate at a healthy quality threshold.",
    status: "completed",
    ownerId: "user-lina-haddad",
    linkedIssueIds: [
      "issue-sentra-referral-flow",
      "issue-sentra-referral-cohort-review",
    ],
    progress: 100,
    confidence: 91,
    confidenceTrend: "stable",
    impact: "high",
    riskLevel: "low",
    targetMetric: "Referral activation quality above 80%",
    aiInsightIds: [],
    createdAt: "2026-04-22T09:00:00.000Z",
    updatedAt: "2026-05-23T16:00:00.000Z",
  },
  {
    id: "roadmap-reson8-broadcast-loop",
    ventureId: "venture-reson8",
    title: "Broadcast Collaboration Loop",
    description:
      "Preserve the stop decision from a broadcast-style loop that failed to produce retained-creator signal.",
    timeframe: "later",
    goal: "Keep the low-signal path closed and fold learnings into retention validation.",
    status: "killed",
    ownerId: "user-maya-rodriguez",
    linkedIssueIds: ["issue-reson8-archive-broadcast-test"],
    progress: 18,
    confidence: 24,
    confidenceTrend: "declining",
    impact: "low",
    riskLevel: "medium",
    targetMetric: "Creator return intent signal below threshold",
    aiInsightIds: ["insight-reson8-kill-broadcast"],
    createdAt: "2026-04-24T09:00:00.000Z",
    updatedAt: "2026-05-22T15:00:00.000Z",
  },
  {
    id: "roadmap-internal-metrics-dashboard",
    ventureId: "venture-internal-ops",
    title: "Studio Metrics Dashboard",
    description:
      "Define a lean metrics surface for portfolio review without expanding into BI reporting.",
    timeframe: "next",
    goal: "Improve portfolio review focus without adding reporting overhead.",
    status: "planned",
    ownerId: "user-sarah-chen",
    linkedIssueIds: ["issue-internal-metrics-dashboard"],
    progress: 28,
    confidence: 71,
    manualConfidence: 71,
    confidenceTrend: "stable",
    impact: "medium",
    riskLevel: "low",
    targetMetric: "Portfolio review prep time -20%",
    aiInsightIds: [],
    createdAt: "2026-05-22T09:00:00.000Z",
    updatedAt: "2026-05-24T11:00:00.000Z",
  },
]
